...
#' Here, we merge the data files together and then do a seasonal split, based on
#' when summer towlanes are active. For the 2019 Crabber Towboat Lane Agreement, 
#' year-round towlanes are always in effect, and summer towlanes are additional lanes
#' available for vessel transit from April 15 to November 24.
#' 
...
dat <- rbind(st_read(here("data", "large_data", "AISTracks_2020_VT_52.shp")),
             st_read(here("data", "large_data", "AISTracks_2020_VT_32.shp")),
             st_read(here("data", "large_data", "AISTracks_2020_VT_31.shp")))

dat_sm <- dat %>% filter(StartDt >= "2020-04-15" & EndDate <= "2020-11-24")
dat_yr <- dat %>% filter(StartDt <  "2020-04-15" | EndDate >  "2020-11-24")
